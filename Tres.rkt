      ...
      (let ([u (nodo-vertice temp)])
        (cond
          [(= (vector-ref visitado u) 0)
           (if (DFS-ciclo grafo u visitado v)
               #t
...
(define (es-arbol? grafo)
  (cond
    ...
    [else
     (let ([visitado (make-vector (grafo-num-vertices grafo) 0)])
       (if (DFS-ciclo grafo 0 visitado -1)
           #f
           (for/and ([i (in-range (grafo-num-vertices grafo))])
             (not (= (vector-ref visitado i) 0)))))]))
...
(define (es-bipartito? grafo)
  (let ([color (make-vector (grafo-num-vertices grafo) -1)])
                  ...
                  (let ([u (nodo-vertice temp)])
                    (cond
                      [(= (vector-ref color u) -1)
                       ...
                       (set! cola (append cola (list u)))
                       (set! fin (+ fin 1))
                       (recorrer-adyacentes (nodo-siguiente temp))]
    ...
    (let ([resultado #t])
      (for ([inicio (in-range (grafo-num-vertices grafo))])
        (when (and resultado (= (vector-ref color inicio) -1))
          (set! resultado (verificar-vertice inicio))))
      resultado)))
